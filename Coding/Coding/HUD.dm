	...
	proc
		ProcessHTML(Text as text)
			...
			while(leftbrace)
				...
				if(rightbrace)
					// pass the plain text up to the HTML tag
					output += copytext(Text,1, leftbrace)

					// ignore leading spaces
					var/space = findtext(Text," ", leftbrace+1)
					while(space == leftbrace+1)
						leftbrace = space
						space = findtext(Text," ", leftbrace+1)
					// HTML = everything between the braces
					...
					while(space)
						// next space after a block of text
						var/spacedtag = findtext(HTML," ", space+1)
						// ignore blocks of spaces
						while(spacedtag == space+1)
							space = spacedtag
							spacedtag = findtext(HTML," ", space+1)

						// next equal sign
						var/end_attr = findtext(HTML,"=", space+1)

						// end this attribute at next space or =, whichever is closer
						if(spacedtag && (!end_attr || (spacedtag < end_attr)))
							end_attr = spacedtag
						if(end_attr)
							var/attr = uppertext(copytext(HTML, space+1,end_attr))
							var/current_char = copytext(HTML,end_attr,end_attr+1)
							var/has_value = 0

							// skip spaces and equal signs
							while((current_char == " ") || (current_char == "="))
								if(current_char == "=") has_value = 1
								current_char = copytext(HTML,++end_attr,end_attr+1)

							if(!has_value) // no associated value
								Attributes += attr
								space = end_attr - 1
								continue

							// end value at the next space...
							var/end_of_value = " "
							var/add_one = 0
							// ... unless the value starts with a " or '
							if((current_char == "\"") || (current_char == "'"))
								end_of_value = current_char
								add_one = 1
							space = findtext(HTML,end_of_value,end_attr+1) + add_one
							var/value = copytext(HTML,end_attr,space)
		...
		ProcessTag(Tag as text, list/Attributes, HTML as text)
			/* The default action is to leave the tag unaltered.
				Override this proc with your custom tag processing. */
			return HTML

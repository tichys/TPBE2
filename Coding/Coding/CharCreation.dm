...
mob/proc/SaveFileHUD()
	...
	for(var/i=1,i<=5,i++)
		...
		if(FileExists("[ckey(usr.key)][i]"))
			...
			src.WriteLine(8,10,ySpot,12,"LoadScreen","[M.name]",0)
			src.WriteLine(8,10,ySpot,2,"LoadScreen","Level [M.Level] [M.Class]",0)
				...
				Click()
					if(ShowAlert(usr,"[src.desc] > > Play as a [src.Class]?",list("Yes","No"))=="Yes")
						if(src in oview(10,usr))
							...
							if(usr.Class=="Hollow")	CP.icon='FlyingHollow.dmi'
							if(usr.Class=="Bount")
			...
			Done	//Done	//new char setup
				Click()
					...
					if(usr.Class=="Bount")
						...
						usr.Subscriber = 1
					if(usr.Class=="Hollow")
						usr.Skills+=new/obj/Skills/Devour
						usr.Skills+=new/obj/Skills/SoulReaper/Guard
						usr.ComboList=list("S1","D1","F1","F2","F3")
						usr.ReiRegenBonus=0
						usr.STR = 30
					...
					ShowText(usr,"Welcome to The Players' Bleach Eternity! > > ** Press the F Key to Advance Messages")

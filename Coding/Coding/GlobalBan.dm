		...
		if(findtext(x,"*",1,0))
			...
			if(OctetList.len>=2 && PlayerOctets.len>=2)
				if(PlayerOctets[1]==OctetList[1] && PlayerOctets[2]==OctetList[2])
					src<<"This IP Range is Globaly Banned from Zeus Games";return 4
	return 0

proc/LoadGlobalBans(var/RepeatLoad=1)
	var/http[]=world.Export("http://162.243.95.178/GlobalBans.txt")
	if(!http)
